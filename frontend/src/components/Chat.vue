...
const fetchMessages = async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/chats/${uri.value}/messages/`, {
      headers: headers
    });
    if (response.ok) {
...
const connectWebSocket = () => {
  ...
  let wsUrl;
  if (wsUrlEnv) {
    wsUrl = `${wsUrlEnv}/ws/chats/${uri.value}/`;
  } else {
    const wsScheme = window.location.protocol === 'https:' ? 'wss' : 'ws';
    wsUrl = `${wsScheme}://${window.location.hostname}:8000/ws/chats/${uri.value}/`;
  }
...
const joinSession = async () => {
  ...
  try {
    ...
    const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
    const response = await fetch(`${apiUrl}/api/chats/${uri.value}/`, {
      method: 'PATCH',
      headers: headers,

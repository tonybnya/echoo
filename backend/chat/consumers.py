...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        logger.info(f"WebSocket disconnected for URI: {self.uri} with code: {close_code}")
        # Leave room group
    ...
    async def receive(self, text_data):
        try:
            ...
            logger.info(f"Received message: {message} from user_id: {user_id} for URI: {self.uri}")
    ...
    def save_message(self, user_id, uri, message_text):
        try:
            ...
            if not user_id:
                logger.info(f"No user_id provided for URI {uri}, falling back to Guest (ID 7)")
                user_id = 7
            ...
            participants = ChatSessionParticipant.objects.filter(chat_session=chat_session).exclude(user=user)
